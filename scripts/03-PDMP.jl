...
plot_PDMP!(ax, ϕ, Πp, ϕ1, var=:x1, label=L"+", linestyle=:dash)
plot_PDMP!(ax, ϕ, Πm, ϕ2, var=:x2, label=L"-", linestyle=:dash)
...
plot_PDMP!(inset_ax, ϕ, Πp .+ Πm, [ϕ1, ϕ2], var=:s, linewidth=0.6, linestyle=:dash)
plot_theory!(inset_ax, x, Π0LNA, var=:s, linewidth=0.6)
xlims!(ax, TK2.bounds(approx) .+ (-0.1, 0.1))
limits!(inset_ax, ϕ1 - 0.2, ϕ2 + 0.3, 0, maximum(Π0LNA) * 1.1)
f2

if SAVEFIG
    save_fig_and_conf("$SCRIPTNAME-LNA", f1, conf)
    save_fig_and_conf("$SCRIPTNAME-LNA-prop", f2, conf)
end
